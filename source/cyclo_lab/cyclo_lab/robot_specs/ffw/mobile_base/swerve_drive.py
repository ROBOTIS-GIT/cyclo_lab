...
class SwerveDriveController:
    ...
    def compute_commands(
        ...
        *,
        ...
        for index, module in enumerate(self.modules):
            current_steering_angle = self._current_steering(index, current_steering_positions, enabled_open_loop)
            ...
            target_steering_joint_angle = normalize_angle(target_steering_angle_robot - module.angle_offset)
            ...
            angle_diff_after_opt = shortest_angular_distance(current_steering_angle, optimized_steering_angle)
            crosses_boundary = (
            ...
            if not command_is_zero:
                direction_changed = wheel_rotation_direction != self.previous_wheel_rotation_direction[index]
            ...
            if not command_is_zero:
                ...
                if phase == ReversalPhase.DECELERATING:
                    ...
                    steering_target_for_this_cycle = limited_steering_cmd
                    self.wheel_speed_scale[index] = 0.0
                    steering_error = abs(shortest_angular_distance(current_steering_angle, limited_steering_cmd))
        ...
        for index, module in enumerate(self.modules):
            wheel_velocity = (
                final_wheel_velocity_commands[index] * wheel_saturation_scale_factor if all_steering_aligned else 0.0
            )
    ...
    def _select_steering_solution(
        ...
        for angle, direction in candidates:
            if module.steering_limit_lower - _LIMIT_EPSILON <= angle <= module.steering_limit_upper + _LIMIT_EPSILON:
                clamped_angle = max(
                    module.steering_limit_lower,
...
def _compute_direct_swerve_commands(
    ...
    *,
    ...
    for index, module in enumerate(modules):
        current_steering = float(current_steering_positions[index]) if current_steering_positions is not None else 0.0
        if command_is_zero:
            commands.append(SwerveModuleCommand(module.steering_joint, module.wheel_joint, current_steering, 0.0, 0.0))
            continue
